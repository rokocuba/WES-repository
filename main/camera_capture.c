...
#define DEFAULT_CAPTURE_BUTTON_GPIO         GPIO_NUM_36
#define DEFAULT_CAPTURE_BUTTON_ACTIVE_LEVEL 1
#define DEFAULT_BUTTON_POLL_MS              20
#define DEFAULT_BUTTON_DEBOUNCE_MS          40
...
static camera_capture_config_t s_cfg = {
    .enable_display = true,
    .enable_button_trigger = true,
    .button_gpio = DEFAULT_CAPTURE_BUTTON_GPIO,
    .button_active_level = DEFAULT_CAPTURE_BUTTON_ACTIVE_LEVEL,
    .button_poll_ms = DEFAULT_BUTTON_POLL_MS,
    .button_debounce_ms = DEFAULT_BUTTON_DEBOUNCE_MS,
};
static camera_capture_frame_cb_t s_frame_cb = NULL;
...
static esp_err_t init_uart1(void) {
...
static void display_received_image(uint8_t* image_buffer, size_t image_size) {
    ...
    camera_capture_frame_t frame = {
        .data = image_buffer,
        .size = image_size,
        .width = (uint16_t)decoded_w,
        .height = (uint16_t)decoded_h,
        .components = components,
        .sequence = ++s_frame_sequence,
    };

    if (s_frame_cb != NULL) {
        s_frame_cb(&frame, s_frame_cb_ctx);
    }

    if (!s_cfg.enable_display) {
        free(image_buffer);
        return;
    }
...
static void request_and_receive_image(void) {
    ...
    if (!wait_for_marker(FRAME_END, 3000)) {
        ...
        display_received_image(image_buffer, image_size);
    }
}
...
esp_err_t camera_capture_request_once(void) {
    ...
    if (pdTRUE != xSemaphoreTake(s_capture_mutex, pdMS_TO_TICKS(3000))) {
...
static void image_task(void *pvParameters) {
    ...
    int last_level = gpio_get_level(s_cfg.button_gpio);
    TickType_t last_press_tick = 0;
    TickType_t debounce_ticks = pdMS_TO_TICKS(s_cfg.button_debounce_ms);

    for (;;) {
        int level = gpio_get_level(s_cfg.button_gpio);

        if (level == s_cfg.button_active_level && last_level != s_cfg.button_active_level) {
            TickType_t now = xTaskGetTickCount();
            if ((now - last_press_tick) >= debounce_ticks) {
                last_press_tick = now;
                ESP_LOGI(TAG_BUTTON, "Button pressed, requesting picture");
                (void)camera_capture_request_once();
            }
        }

        last_level = level;
        vTaskDelay(pdMS_TO_TICKS(s_cfg.button_poll_ms));
    }
}
...
esp_err_t camera_capture_start(const camera_capture_config_t *config) {
    ...
    if (s_capture_mutex == NULL) {
        ...
        if (s_capture_mutex == NULL) {
            ESP_LOGE(TAG_REQUESTER, "Failed to create capture mutex");
            return ESP_FAIL;
        }
    }

    if (!s_cfg.enable_button_trigger) {
        ESP_LOGI(TAG_BUTTON, "Button trigger disabled; use camera_capture_request_once() instead");
        return ESP_OK;
    }
    ...
    BaseType_t ok = xTaskCreate(image_task, "image_task", 8192, NULL, 5, NULL);
    if (ok != pdPASS) {
        ESP_LOGE(TAG_REQUESTER, "Failed to create image task");
        return ESP_FAIL;
    }
